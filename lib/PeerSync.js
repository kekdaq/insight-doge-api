...
PeerSync.prototype.handleBlock = function(info) {
  ...
  // Switch from read block directly to historical sync.
  //
  // Reason: Since dogecoin 1.8 auxpow the block is encoded differently and dogecore isn't able to read it
  // anymore. The historical sync still works via rpc and therefore is used for the time being.
  //
  // FIXME: Fix block parsing for dogecoin 1.8 auxpow blocks.
  //
  self.historicSync.start({forceRPC:0}, function(){
...
PeerSync.prototype.run = function() {
  var self = this;
  this.logger.info('peer sync starting up');
